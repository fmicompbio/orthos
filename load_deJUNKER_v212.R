...
DF <-  data.frame(idx=1:length(scores), score=scores, ACC=names(scores) )       
...
dens.plot <- ggpubr::ggdensity(DF,"score", fill="#33638DFF") + clean_theme() +
    geom_point(data=DF %>%filter(score >= min.score),aes(x=score, y=0),size=1.5,color="red") +
    ggpubr::rotate() +  theme(plot.margin = unit(c(1, 0, 1, 0), "cm")) +blank.theme

manh.plot <- ggplot(data=DF, aes(x=idx,y=score )) + geom_bin2d(bins=200) + 
    scale_fill_continuous(type = "viridis")+ theme_bw() +
    geom_point(data=subset(DF, score >=min.score),color="red",size=1.5 )+ 
    geom_text_repel(data=subset(DF, score >=min.score ),aes(x=idx,y=score ,label=ACC ),size=3  )+
    theme(legend.position='none') +  theme(plot.margin = unit(c(1, 0, 1, 0), "cm"))+
    ggpubr::rremove("x.ticks") + theme(panel.border=element_blank())

P <- cowplot::plot_grid(manh.plot,NULL,dens.plot,ncol=3,align="hv", 
