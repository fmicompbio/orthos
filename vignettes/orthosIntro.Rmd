...
database of uniformly processed RNAseq experiments. 
...
1. Decomposing one or several contrasts into their corresponding specific and
   polytopic fractions using the `decomposeVar()` function and
...
```{r}
MKL1_human <- readRDS(system.file("data", "GSE215150_MKL1_Human.rds",
                                  package = "orthos"))
head(MKL1_human)
dim(MKL1_human)
MKL1_mouse <- readRDS(system.file("data", "GSE215150_MKL1_Mouse.rds",
                                  package = "orthos"))
head(MKL1_mouse)
dim(MKL1_mouse)
```
...
In both modes the rownames of `M` (and `MD` if specified) need to correspond 
